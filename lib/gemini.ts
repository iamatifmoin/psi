...
export type DirectorDecision = { clips: Array<{ start_time: number; end_time: number; label?: string }>; intro_narration: string; outro_narration: string };
...
export async function directHighlight(timeline: TimelinePayload): Promise<DirectorDecision> {
  ...
  if (!response.ok) throw new Error(`Gemini director failed (${response.status}).`);
